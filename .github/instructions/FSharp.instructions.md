...
- `String.Contains` has no `StringComparison` overload on `netstandard2.0`; the span one has. Spell it `text.AsSpan().Contains(value.AsSpan(), StringComparison.Ordinal)`.
- A slice of a string that is only inspected – compared, trimmed, scanned, matched against a prefix – is a `ReadOnlySpan<char>` (`text.AsSpan(start, length)`), not a `Substring`: the substring allocates a copy per call, the span does not. `System.Memory` is referenced by `src/Compiler` on `netstandard2.0`, so `MemoryExtensions` is available everywhere – including its `StringComparison` overloads of `StartsWith`, `EndsWith`, `Equals`, `CompareTo`, `IndexOf` and `Contains` for `ReadOnlySpan<char>`. The rule above applies to spans unchanged: `s.AsSpan().TrimStart(' ').StartsWith("//".AsSpan(), StringComparison.Ordinal)`. The overloads without one are the generic element-wise `ReadOnlySpan<'T>` ones – ordinal for `char` by accident, not by statement. `name.AsSpan(1, name.Length - 4)` handed to a recursive checker (`QuickParse.fs`) needs no copy either. Materialise with `Substring`/`ToString()` only for the value that leaves the function or is stored. A slice that must outlive the stack frame – kept in a record, captured by a closure, returned from a member – is `ReadOnlyMemory<char>` (`text.AsMemory(...)`), never a `ReadOnlySpan<char>` (a byref-like value cannot be stored). To compare two slices without building either, use `String.CompareOrdinal(a, aIndex, b, bIndex, length)` or `spanA.Equals(spanB, StringComparison.Ordinal)`.
